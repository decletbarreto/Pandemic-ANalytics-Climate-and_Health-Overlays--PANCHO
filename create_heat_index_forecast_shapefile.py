...
print("Starting process: Create Heat Index Shapefile.") 
...
#root_dir  = r"D:\Users\climate_dashboard\Documents\SuperPANCHO"
root_dir = r'D:\UCS_projects\climate_shop\SuperPANCHO'

# Local variables:   
data_dir         = "data"
tmp_dir          = os.path.join(root_dir, data_dir, "tmp", "HI_forecast")
output_files_dir = os.path.join(root_dir, data_dir, "output")
input_files_dir  = os.path.join(root_dir, data_dir, "input")
noaa_heat_index_base_url = "https://ftp.wpc.ncep.noaa.gov/shapefiles/heatindex/" 
historical_archive_dir = os.path.join(root_dir, data_dir, "historical/HI")
...
try:
    ...
    msg = "Starting process: Create Heat Index shapefile."
    print(msg)
    logging.info(msg)
    ...
    max_hi_forecast_url_072  = noaa_heat_index_base_url + date_two_days_ago_formatted + "/maxhi_" + date_two_days_ago_formatted + "f072.tar" 
    max_hi_forecast_url_096  = noaa_heat_index_base_url + date_two_days_ago_formatted + "/maxhi_" + date_two_days_ago_formatted + "f096.tar" 
    max_hi_forecast_url_120  = noaa_heat_index_base_url + date_two_days_ago_formatted + "/maxhi_" + date_two_days_ago_formatted + "f120.tar" 
    max_hi_forecast_url_144  = noaa_heat_index_base_url + date_two_days_ago_formatted + "/maxhi_" + date_two_days_ago_formatted + "f144.tar" 
    max_hi_forecast_url_168  = noaa_heat_index_base_url + date_two_days_ago_formatted + "/maxhi_" + date_two_days_ago_formatted + "f168.tar"
    ...
    maxhi_counties_join_final= os.path.join(output_files_dir, "maxhi_counties_join_final")    
    historical_maxhi_counties_join_final= os.path.join(historical_archive_dir, "maxhi_counties_" + str(datetime.date.today().strftime("%Y%m%d")))
    ...
    try:
        ...
        msg = "\tCould not download " + url + " to " + file + "."
        print(msg)
        logging.info(msg)
        sys.exit()
    ...
    for shp in shp_list:
        ...
        #select Heat Index features contained by CONUS counties shapefile
        try:
            msg = "\tSelecting Heat Index features contained in CONUS counties shapefile." 
            print(msg)
            logging.info(msg)
            ...
            msg = "Failed Select Layer by Location. Exiting."
            print(msg)
            logging.info(msg)
            print(arcpy.GetMessages())
            print("input: " + counties_shp)
            print("output: " + max_hi_forecast_shp)
            sys.exit()
    ...
    #zipping to SuperPANCHO output folder
    for a_file in files:
        try:
    ...
    msg = "zipping shapefile to historical archive..."
    print(msg)
    logging.info(msg)
    #zipping to SuperPANCHO historical archive folder
    historical_zip_filename = os.path.join(historical_archive_dir, historical_maxhi_counties_join_final) + ".zip"
    historical_zip = zipfile.ZipFile(historical_zip_filename, 'w')
    for a_file in files:
        try:
            historical_zip.write(filename = os.path.join(tmp_dir, a_file), arcname = os.path.basename(a_file), compress_type=zipfile.ZIP_DEFLATED)
            msg = "Zipped to " + historical_zip_filename
            print(msg)
            logging.info(msg)
        except:
            msg = "can't zip " + a_file
